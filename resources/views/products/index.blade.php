...
<div x-data="{ openCreate: false, openEdit: false, editProduct: null }">
    ...
    <div class="flex flex-wrap items-center justify-between gap-3 mb-5">
        ...
        <div class="flex gap-2">
            <button x-data @click="$dispatch('open-import-modal')"
                    class="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium rounded-xl border transition-colors"
                ...
                <tbody>
                    ...
                    <tr class="border-t transition-colors {{ $belowMin ? 'border-red-100 dark:border-red-900/30 bg-red-50/50 dark:bg-red-900/5 hover:bg-red-50 dark:hover:bg-red-900/10' : 'border-gray-50 dark:border-gray-700/60 hover:bg-gray-50/70 dark:hover:bg-white/[0.025]' }}">
                        ...
                        <td class="px-5 py-3.5 font-medium text-sm {{ $belowMin ? 'text-red-700 dark:text-red-400' : '' }}" style="{{ $belowMin ? '' : 'color:var(--text-primary)' }}">
                            <div class="flex items-center gap-1.5">
                                {{ $product->name }}
                                @if($belowMin)
                                <i class="ph ph-warning-circle text-sm text-red-500" title="Dưới ngưỡng tồn tối thiểu ({{ number_format($product->min_stock,0,',','.') }})"></i>
                                @endif
                            </div>
                        </td>
                    ...
                    <tr>
                        <td colspan="8" class="px-5 py-16 text-center">
                            ...
                            <p class="text-sm" style="color:var(--text-muted)">Chưa có sản phẩm nào</p>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
    ...
    <div x-data="{ open: false }"
        ...
        <div class="modal-panel relative w-full max-w-md p-6"
            ...
            <div class="flex items-center justify-between mb-5">
                <div class="flex items-center gap-2.5">
                    <div class="w-8 h-8 rounded-lg flex items-center justify-center" style="background:rgba(22,163,74,0.10)">
                        <i class="ph ph-file-arrow-up text-sm" style="color:#16a34a"></i>
                    </div>
            ...
            <p class="text-xs mb-4 p-3 rounded-xl" style="color:var(--text-muted);background:var(--surface-bg)">
            ...
            <form action="{{ route('products.import') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="file" name="file" required accept=".xlsx,.xls,.csv" class="form-input">
                </div>
                <div class="flex justify-end gap-2">
                    ...
                    <button type="submit" class="px-4 py-2 text-sm font-medium text-white rounded-xl" style="background:#16a34a" onmouseover="this.style.background='#15803d'" onmouseout="this.style.background='#16a34a'">
                        <i class="ph ph-upload mr-1"></i> Import
                    </button>
                </div>
            </form>
        </div>
