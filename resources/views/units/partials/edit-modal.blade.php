<div x-show="openEdit" x-transition class="fixed inset-0 z-50 flex items-center justify-center p-4" style="display:none">
    ...
    <div class="relative bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-sm">
        <div class="flex items-center justify-between p-5 border-b border-gray-200 dark:border-gray-700">
            <h3 class="text-base font-semibold text-gray-900 dark:text-white">Sửa đơn vị tính</h3>
            <button @click="openEdit = false" class="text-gray-400 hover:text-gray-600"><i class="bi bi-x-lg"></i></button>
        </div>
        <template x-if="editUnit">
        <form :action="`/units/${editUnit.id}`" method="POST" class="p-5 space-y-4">
            @csrf @method('PUT')
            <div>
                ...
                <p class="mt-1 text-xs text-gray-400">Chữ in hoa, số, dấu gạch dưới.</p>
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                    Tên đơn vị <span class="text-red-500">*</span>
                </label>
                <input type="text" name="name" :value="editUnit.name" required maxlength="50"
