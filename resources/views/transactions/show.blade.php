    ...
    <div class="lg:col-span-3 bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-5">
        <div class="flex items-start justify-between mb-4">
            <div>
                <div class="flex items-center gap-3 mb-1">
                    ...
                    @endphp
                    <span class="inline-flex px-2 py-0.5 rounded text-xs font-medium {{ $typeBadge }}">{{ $typeLabel }}</span>
                    <span class="inline-flex px-2 py-0.5 rounded text-xs font-medium {{ $statusBadge }}">{{ $statusLabel }}</span>
                </div>
                ...
                @if($transaction->isDraft())
                    ...
                    <form action="{{ route('transactions.submit', $transaction) }}" method="POST">
                        ...
                        <button type="submit" class="inline-flex items-center gap-1.5 px-3 py-2 text-sm bg-yellow-500 hover:bg-yellow-600 text-white rounded-lg">
                            <i class="bi bi-send"></i> Submit chờ duyệt
                        </button>
                    </form>
                    @endcan
                    ...
                    <form action="{{ route('transactions.destroy', $transaction) }}" method="POST"
                        ...
                        @csrf @method('DELETE')
                        <button type="submit" class="inline-flex items-center gap-1.5 px-3 py-2 text-sm bg-red-50 text-red-600 hover:bg-red-100 rounded-lg border border-red-200">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            <div>
                ...
                <p class="font-medium">{{ $transaction->supplier?->name ?? $transaction->destination?->name ?? '—' }}</p>
            </div>
            <div>
        ...
        <div class="mt-4 p-3 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg text-sm text-red-700 dark:text-red-400">
        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                ...
                <tr>
                    ...
                    <td class="px-4 py-3 text-right">{{ number_format($detail->qty, 0, ',', '.') }}</td>
                    @if($transaction->type === 'IN')
                    ...
                    <td class="px-4 py-3 text-right font-medium">{{ number_format($detail->amount, 0, ',', '.') }}đ</td>
                    @endif
                </tr>
                @endforeach
            </tbody>
...
<div class="lg:col-span-3 bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden"
     x-data="{
         lightbox: false,
         current: 0,
         images: {{ $transaction->attachments->where('mime_type', 'like', 'image/%')->values()->map(fn($a) => ['url' => $a->url, 'name' => $a->original_name])->toJson() }},
            ...
            <div class="relative group flex-shrink-0">
                ...
                <a href="{{ $att->url }}" target="_blank"
                   class="flex flex-col items-center justify-center w-20 h-20 rounded-xl ring-1 ring-gray-200 dark:ring-gray-600 hover:ring-primary-400 transition-all bg-gray-50 dark:bg-gray-700 gap-1">
                    ...
                    <span class="text-xs text-gray-500 truncate w-16 text-center px-1">{{ $att->original_name }}</span>
                </a>
                @endif

                {{-- Size tooltip --}}
                <span class="absolute -bottom-5 left-1/2 -translate-x-1/2 text-xs text-gray-400 whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
                    {{ $att->human_size }}
                </span>
...
<div x-data="{ open: false }" @open-reject.window="open = true">
    <div x-show="open" x-transition class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div class="relative bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-md p-5">
            <h3 class="font-semibold text-gray-900 dark:text-white mb-4">Từ chối phiếu {{ $transaction->code }}</h3>
            <form action="{{ route('transactions.reject', $transaction) }}" method="POST">
                ...
                <div class="flex justify-end gap-2">
                    <button type="button" @click="open = false" class="px-4 py-2 text-sm border border-gray-300 rounded-lg">Hủy</button>
                    <button type="submit" class="px-4 py-2 text-sm bg-red-600 text-white rounded-lg hover:bg-red-700">Từ chối</button>
                </div>
            </form>
        </div>
    </div>
