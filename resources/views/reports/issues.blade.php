...
<div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 overflow-hidden"
    ...
    <form method="GET" class="p-4 flex flex-wrap gap-3 items-end" style="border-bottom:1px solid var(--surface-border)">
        ...
        <div class="flex flex-col gap-1">
            <label class="text-[11px] font-semibold uppercase tracking-wide" style="color:var(--text-muted)">Điểm nhận</label>
    ...
    <div class="px-5 py-2.5 flex gap-6 text-sm" style="background:var(--surface-bg);border-bottom:1px solid var(--surface-border)">
        ...
        <span style="color:var(--text-muted)">Tổng SL: <strong style="color:var(--text-primary)">{{ number_format($rows->sum(fn($r) => abs($r->qty)), 0, ',', '.') }}</strong></span>
        <span style="color:var(--text-muted)">Tổng giá trị: <strong style="color:#ef4444">{{ number_format($rows->sum(fn($r) => abs($r->qty) * $r->cost_price), 0, ',', '.') }}đ</strong></span>
    </div>
    @endif
        ...
        <table class="w-full text-sm text-left whitespace-nowrap">
            <thead>
                <tr style="background:var(--surface-bg);border-bottom:1px solid var(--surface-border)">
                    ...
                    <th class="px-5 py-3 text-left text-[11px] font-semibold uppercase tracking-wide" style="color:var(--text-muted)">Điểm nhận</th>
                    ...
                    <th class="px-5 py-3 text-right text-[11px] font-semibold uppercase tracking-wide" style="color:var(--text-muted)">SL xuất</th>
            ...
            <tbody>
                ...
                <tr class="border-t border-gray-50 dark:border-gray-700/60 hover:bg-gray-50/70 dark:hover:bg-white/[0.025] transition-colors">
                    ...
                    <td class="px-5 py-2.5">
                        @php
                        $destName = $row->transaction?->destination?->name;
                        $badgeClass = match(true) {
                            $destName === 'Kho 43' => 'badge-blue',
                            $destName === 'Kho 44' => 'badge-purple',
                            default => 'badge-gray',
                        };
                        @endphp
                    ...
                    <td class="px-5 py-2.5 text-right font-medium tabular-nums text-sm" style="color:var(--text-primary)">{{ number_format($qty, 0, ',', '.') }}</td>
                    <td class="px-5 py-2.5 text-right text-xs tabular-nums" style="color:var(--text-secondary)">{{ number_format($row->cost_price, 0, ',', '.') }}</td>
                    <td class="px-5 py-2.5 text-right font-medium tabular-nums text-sm" style="color:var(--text-primary)">{{ number_format($value, 0, ',', '.') }}đ</td>
                </tr>
                @empty
                <tr>
            ...
            <tfoot>
                <tr style="background:var(--surface-bg);border-top:2px solid var(--surface-border)">
                    ...
                    <td class="px-5 py-3 text-right text-sm font-bold tabular-nums" style="color:var(--text-primary)">{{ number_format($rows->sum(fn($r) => abs($r->qty)), 0, ',', '.') }}</td>
                    <td></td>
                    <td class="px-5 py-3 text-right text-sm font-bold tabular-nums" style="color:#ef4444">{{ number_format($rows->sum(fn($r) => abs($r->qty) * $r->cost_price), 0, ',', '.') }}đ</td>
                </tr>
            </tfoot>
            @endif
        </table>
